...
export const discountApproveForm = createAsyncThunk<
  ApproveFormData,
  { formId: number; data: ApproveFormData }
>("discount/approveForm", async ({ formId, data }, { rejectWithValue }) => {
  try {
    const token = localStorage.getItem("token");
    const response = await API.post<ApproveFormData>(
      `/request_discount/approve/${formId}`,
      data,
      {
        headers: {
...
export const refreshFiles = createAsyncThunk(
  "discount/refreshFiles",
  async ({ token, id }: { token: string; id: string }, { rejectWithValue }) => {
    try {
      const response = await API.get(`/api/request_discount/show/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(refreshFiles.fulfilled, (state, action) => {
        if (state.detailData) {
          state.detailData.files = action.payload;
        }
      })
      .addCase(refreshFiles.rejected, (state, action) => {
        console.error("Error refreshing files:", action.payload);
      });
  },
});
