...
import { generalEvaData, searchEvaData } from "../../../api/ME/eva";
// src/assets/css/style.css

const EvaIndex: React.FC = () => {
  const formId = 4;
  ...
  useEffect(() => {
    const cached = sessionStorage.getItem("eva_cache");
    if (cached) {
      const parsed = JSON.parse(cached);
      parsed.activePage = activePage;
      sessionStorage.setItem("eva_cache", JSON.stringify(parsed));
    }
  }, [activePage]);
  useEffect(() => {
    const cached = sessionStorage.getItem("eva_cache");
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const data = await generalEvaData(token);
      setGeneralData({
        meta: {
          authenticatedUser: data?.authenticatedUser,
          branch: data?.branch,
  ...
  const handleSearch = async () => {
    ...
    try {
      const results = await searchEvaData(token, searchTerm);
  ...
  const handleRestart = async () => {
    sessionStorage.removeItem("eva_cache");
    ...
    navigate(`/evaporator/${formId}`, { replace: true });
  };
      ...
      return (
        ...
        >
          ...
          <Table.Td className="flex flex-justify gap-3 items-center">
            <Link
              to={`/me_evaporator_detail/${element.id ?? ""}`}
              className="contents"
            >
              {element.form_doc_no ?? "-"}
            </Link>
    ...
    <div>
      <div className="p-6 bg-white">
        ...
        <div className="flex justify-between mr-4">
          <h2 className="text-xl font-semibold">Evaporator Form</h2>
