...
import { getWaterTankData, searchWaterTankData } from "../../../api/ME/WaterTank/watertank";
...
const WaterTankIndex: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      sessionStorage.setItem("watertank_cache", JSON.stringify(parsed));
    }
  }, [activePage]);
  useEffect(() => {
    const cached = sessionStorage.getItem("watertank_cache");
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const data = await getWaterTankData(token);
  ...
  console.log("SearchTerm>>", waterTankData);
  console.log("createdUser>>", waterTankData?.meta?.createdUser);
  ...
  const handleSearch = async () => {
    ...
    try {
      const results = await searchWaterTankData(token, searchTerm);
      ...
      setWaterTankData((prev) => ({ ...prev, data: results.data }));
  ...
  const handleRestart = async () => {
    ...
    navigate(`/water-tank/${formId}`, { replace: true });
  };
  ...
  const paginatedData = useMemo(() => {
    const start = (activePage - 1) * pageSize;
    return Array.isArray(waterTankData?.data)
      ? waterTankData.data.slice(start, start + pageSize)
      : [];
  }, [waterTankData, activePage]);
  const rows = useMemo(() => {
    return paginatedData?.map((element: indexData, index: number) => {
      const isCopied = copied === element.id;

      const hasUnreadNotification = waterTankData?.meta?.noti_data?.some(
        (item: indexData) =>
      ...
      return (
        ...
        >
          ...
          <Table.Td className="flex flex-justify gap-3 items-center">
            <Link
              to={`/water-tank-detail/${element.id ?? ""}`}
              className="contents"
            >
              {element.form_doc_no ?? "-"}
            </Link>
  ...
  const showLoading = loading || !waterTankData;
  if (showLoading) {
    return (
    ...
    <div>
      <div className="p-6 bg-white">
        ...
        <div className="flex justify-between mr-4">
          <h2 className="text-xl font-semibold">Water Tank Form</h2>
          {waterTankData?.meta?.createdUser === true && (
            <Link
              to="/water-tank/create"
              state={{ formId: formId }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-4 mb-6 text-sm mt-4 items-center">
          ...
          {waterTankData?.meta?.authenticatedUser?.role_id == 1 ? (
            <div className="flex flex-col">
              <label
                htmlFor="status"
                className="mb-1 font-medium text-gray-700"
          ...
          <div className="flex flex-col">
            ...
            {waterTankData?.meta?.authenticatedUser?.emp_id == "000-000046" ||
            waterTankData?.meta?.authenticatedUser?.emp_id == "000-000071" ||
            waterTankData?.meta?.authenticatedUser?.emp_id == "000-000024" ||
            waterTankData?.meta?.authenticatedUser?.emp_id == "000-000067" ? (
              <Select
                id="branch_id"
                searchable
                clearable
                value={searchTerm.branch_id}
                data={waterTankData?.meta?.branch?.map((item: indexData) => ({
              ...
              <Select
                ...
                clearable
                value={searchTerm.branch_id}
                data={waterTankData?.meta?.user_branches?.map(
                  (item: indexData) => ({
      ...
      <div className="mx-auto mt-6  items-center  px-4">
        <Pagination
          total={Math.ceil((waterTankData?.data?.length ?? 0) / pageSize)}
          value={activePage}
