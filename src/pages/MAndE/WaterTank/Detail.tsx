...
import type {  meGeneratorDataType, meWaterTankDataType } from "../../../utils/meDataUtil/metype";
...
import { waterTankDetailData } from "../../../api/ME/WaterTank/watertank";
import WaterTankTableDetail from "./TableDetail";

const waterTankDetail: React.FC = () => {
  ...
  const fetchData = async (id: string | number) => {
    ...
    setLoading(true);
    try {
      const data = await waterTankDetailData(token, id);
      setDetailData(data);
    } catch (error) {
            ...
            <div className="bg-white shadow-md rounded-lg p-4 sm:p-6 mb-6 w-full ">
              ...
              <NavPath
                segments={[
                  ...
                  {
                    path: `/water-tank/${detailData?.subForm?.sub_form_id}`,
                    label: "Water Tank",
                  },
                  {
                    path: `/me_water_tank_detail/${id}`,
                    label: "Water Tank Detail",
                  },
                ]}
              />
                  ...
                  {detailData?.generalForm?.status === "Default" && (
                    <Button
                      ...
                      state={{
                        ...
                        waterTankFormId: detailData?.generalForm?.id,
                      }}
...
export default waterTankDetail;
